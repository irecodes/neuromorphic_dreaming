...
class sliderGui():
    ...
    def __init__(self, model=None):
        ...
        self.LogDir= "./LogsBias/" # I would like to have to separate variables one for dir and the other for the filename
        self.LogFileName = "./LogsBias/changes_log"
        ...
        #>>d Init usefful variables
        self.last_updated_state = {}
        self.last_recorded_state = {}

        self.log_changes_flag = False
        self.log_changes_tk_var = tk.IntVar() # IntVar class is like an integer (a number), but works in special ways with other part of the tkinter library. The short (and dense) version is that IntVar allows for bi-directionally binding a variable and a UI element
        #<<d
        ...
        #>>d Init last_updated_state from GUI default slider value --> It's a dict. What are you doing below --> [??1??]
        for bias_name in self.slider_vars.keys():
            self.last_updated_state[bias_name]=self.slider_vars[bias_name].get()
        #<<d
        
        # Init GUI params getting them from Dyn 
            ...
            for chip_id in range(4):
                for core_id in range(4):
                    ...
                    for bias_name in BIAS_NAMES:
                        ...
                        #>>d [??1??] Is it a second init
                        self.last_updated_state[bias_name+":C%dC%d" %(chip_id, core_id)]=value
                        #<<d
        
        
        self.flag_loading_config = False # What is this used for?
    ...
    def slider_callback(self, var_name, idx, mode):
        ...
        #>>d
        self.last_updated_state[var_name]=value
        #<<d
    ...
    #>>d    
    def InitBiasLogFile(self):
        pass

    def _update_rec_flag_state(self):

        if self.log_changes_tk_var.get() and self.log_changes_flag is False:

            self.last_recorded_state = self.last_updated_state.copy()
            #>>m
            os.makedirs(os.path.dirname(self.LogFileName), exist_ok=True)
            self.rec_changes_file = open(f"{self.LogFileName}.txt", "w")
            #<<m
            print("Change log file has been created!")
            self.log_changes_flag = True
        else:
            self.log_changes_flag = False
            self.rec_changes_file.close()
            print("Change log file has been saved!")


    def _gui_release_callback(self, event):
        if self.log_changes_flag:
            for bias_name in self.last_updated_state.keys():
                if self.last_recorded_state[bias_name] != self.last_updated_state[bias_name]:
                    print(bias_name, f"CHANGE: {self.last_recorded_state[bias_name]} -> {self.last_updated_state[bias_name]}")
                    self.rec_changes_file.write(bias_name + f"{self.last_recorded_state[bias_name]} -> {self.last_updated_state[bias_name]}\n")
                self.last_recorded_state[bias_name] = self.last_updated_state[bias_name]

    def _gui_press_callback(self, event):
        pass
    #<<d   
...
def create_core_bias_block(gui):
    ...
    #>>d
    tgl_rec = ttk.Checkbutton(gui.frame_main, text = "Record changes",
                              variable=gui.log_changes_tk_var,
                              onvalue=1,
                              offvalue=0,
                            command=gui._update_rec_flag_state)
    #<<d
    ...
    #>>d
    tgl_rec.grid(row=0, column=2, columnspan=3, sticky='ne', ipady=6, ipadx=6)
    #<<d
...
def create_single_bias_block(gui, parent, bias_name):
    ...
    #>>d
    slider.bind("<ButtonRelease>", gui._gui_release_callback)
    slider.bind("<Button>", gui._gui_press_callback)
    #<<d
